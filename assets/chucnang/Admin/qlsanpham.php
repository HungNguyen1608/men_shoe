...
<form method="POST" action="">
    <input type="text" name="search" placeholder="Tên giày">
...
$sqlCount = "SELECT COUNT(*) as total FROM giay WHERE Tengiay LIKE '%$search%'";
$countResult = $conn->query($sqlCount);
$rowCount = $countResult->fetch_assoc();
$totalRows = $rowCount['total'];
$totalPages = ceil($totalRows / $limit);

$sql = "SELECT * FROM giay WHERE Tengiay LIKE '%$search%' LIMIT $limit OFFSET $offset";
$result = $conn->query($sql);

// Kiểm tra và in ra thông tin
if ($result->num_rows > 0) {
    echo "<table id='banggiay'>";
    echo "<tr>";
    echo "<th>Mã giày</th>";
    echo "<th>Tên giày</th>";
    echo "<th>Mã loại giày</th>";
    echo "<th>Số lượng</th>";
    echo "<th>Đơn giá</th>";
    echo "<th>Màu</th>";
    echo "<th>Hãng giày</th>";
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<input type='hidden' name='Magiay' value='" . $row['Magiay'] . "'>";
        echo "<button type='submit' name='delete'>Delete</button>";
        echo "</form>";
        echo " | ";
        echo "<a href='?page_layout=suattgiay&mataikhoan=" . $mataikhoan . "&Magiay=" . $row['Magiay'] . "'>Edit</a>";
        echo "</td>";
        echo "</tr>";
    }
