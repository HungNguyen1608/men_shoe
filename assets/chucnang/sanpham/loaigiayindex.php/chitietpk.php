    ...
    ?>
         <div  class="prd-item" style="display: flex">
            ...
            <div class="noidungchitiet">
                <h3><a style="text-decoration: none; color: black;" href="index.php?page_layout=chitietsp&Magiay=<?php echo $row['Tengiay'] ?>"><?php echo $row['Tengiay'] ?></a></h3>
                <p><?php if(isset($row['Dongia'])) echo $row['Dongia'] ?></p>
                <p>Hãng giày: <?php if(isset($row['Hanggiay'])) echo $row['Hanggiay'] ?></p>
                <p>Tình trạng: <?php if(isset($row['Thongtin'])) echo $row['Thongtin'] ?></p>
                <p class="price"><span>Giá: <?php if(isset($row['Mau'])) echo $row['Mau'] ?> VNĐ</span></p>
                <?php
                    }
                ?>
                <!--PHP RỎ HÀNG -->
                ...
                <form method="POST" action="">
                    <div class="clear">
                        <div class="sizegiay">
                            ...
                            <a href="giaodienmuahangpk.php?mataikhoan=<?php $mataikhoan = $_GET['mataikhoan']; echo $mataikhoan;?>&Magiay=<?php $magiay = $_GET['Magiay']; echo $magiay ;?>&Soluong=<?php echo isset($_POST['quantity']) ? $_POST['quantity'] : ''; ?>&Size=<?php echo isset($_POST['size']) ? $_POST['size'] : ''; ?>" style="color:#111; background-color: #FFB74D;height: 36px; align-items: center;justify-content: center;display: flex;text-decoration: none;text-align: center;" class="btn btnchitiet" name="muahang" type="submit">Mua ngay</a>
                        </div>
                    </div>
                </form>
                <script>
